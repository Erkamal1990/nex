...
module.exports = {
    ...
    CampaigneList: (req, res) => {
        ...
        jwt.verify(token, config.secret, function(err, decoded) {
            ...
            var sqlQuery;
            if(userType == 2){
                sqlQuery = 'SELECT * FROM campaign LEFT JOIN user ON user.id = campaign.user_id';
            }else{
                sqlQuery = "SELECT * FROM campaign LEFT JOIN user ON user.id = campaign.user_id where campaign.user_id = '"+userId+"'";
            }
    ...
    ChangeCampaigneStatus: (req, res) => {
        ...
        jwt.verify(token, config.secret, function(err, decoded) {
            ...
            var sqlQuery = "UPDATE campaign SET status= '" +statusValue+ "' WHERE campaign_id = '" +campaignId+ "';"
